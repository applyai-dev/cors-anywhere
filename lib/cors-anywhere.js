...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var originForCheck = origin;
    if (origin) {
      var originUrl = url.parse(origin);
      if (originUrl.host) {
        originForCheck = originUrl.protocol + '//' + originUrl.host;
      }
    }

    if (corsAnywhere.originBlacklist.indexOf(originForCheck) >= 0) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was blacklisted by the operator of this proxy.');
      return;
    }

    if (corsAnywhere.originWhitelist.length && corsAnywhere.originWhitelist.indexOf(originForCheck) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
